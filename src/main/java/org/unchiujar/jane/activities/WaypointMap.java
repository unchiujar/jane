...
public class WaypointMap extends SherlockFragmentActivity implements Observer {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Marker marker = mMap.addMarker(new MarkerOptions().position(new LatLng(37.7750, 122.4183))
                .title("San Francisco").snippet("Population: 776733"));
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        waypointModeActive = savedInstanceState.getBoolean(BUNDLE_WAYPOINT_EDITING);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case R.id.where_am_i:
            Log.d(TAG, "XXXXX NOT IMPLEMENTED XXXXX Moving to current location...");
    ...
    private Polyline mWaypointPath = new Polyline(null);
    ...
    public void update(Observable observable, Object data) {
        ...
        mWaypointPath.remove();
        // redraw mWaypointPath between markers
        mWaypointPath = mMap.addPolyline(new PolylineOptions().width(5).color(Color.RED));

        // create list from markers
        Collection<Marker> points = markers.values();
        // creat LatLng list from markers
        ArrayList<LatLng> latLngs = new ArrayList<LatLng>();
        for (Marker point : points) {
            latLngs.add(point.getPosition());
        }
        mWaypointPath.setPoints(latLngs);
    }

    private Marker createMarkerFromWaypoint(Waypoint waypoint) {
        return mMap.addMarker(new MarkerOptions().position(
                LocationUtilities.locationToLatLng(waypoint)).title(waypoint.getInfo()));
    }
}
