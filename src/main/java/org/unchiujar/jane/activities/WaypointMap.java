...
import org.unchiujar.jane.preferences.WaypointSaver;
...
public class WaypointMap extends SherlockFragmentActivity implements Observer,
	...
	// restore waypoints
	List<Waypoint> waypoints = WaypointSaver.load(this);
	...
	if (savedInstanceState == null) {
	    ...
	    SharedPreferences prefs = getSharedPreferences(
		    Preferences.JANE_PREFS, Context.MODE_PRIVATE);
	    ...
	    waypointModeActive = savedInstanceState
		    .getBoolean(WAYPOINT_EDITING_SAVE);
	    zoom = savedInstanceState.getFloat(ZOOM_SAVE, 5f);
	}
	mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(
		mCurrentLat, mCurrentLong), zoom));
    }
	...
	case ADD_MULTIPLE:
	    ...
	    Log.d(TAG, "Displaying waypoints: " + waypoints.size());
		...
		markers.put(i, createMarkerFromWaypoint(point));
		i++;
	    }
	...
	drawPathsBetweenWaypoints();
	// save waypoints state
	WaypointSaver.save(this, waypointManager.getWaypoints());
    }
